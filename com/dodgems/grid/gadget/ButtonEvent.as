...
package com.dodgems.grid.gadget
{
    ...
    public class ButtonEvent extends MouseEvent
    {
         ...
         */
        public static function makeButton(object:InteractiveObject, includeCallbacks:Boolean = false):InteractiveObject
        {
            ...
            if(includeCallbacks) {
                ...
                object.addEventListener(ButtonEvent.PRESS, onCallbackEvent);
                object.addEventListener(ButtonEvent.DRAG, onCallbackEvent);
                object.addEventListener(ButtonEvent.ROLL_OVER, onCallbackEvent);
                object.addEventListener(ButtonEvent.ROLL_OUT, onCallbackEvent);
                object.addEventListener(ButtonEvent.DRAG_OVER, onCallbackEvent);
                object.addEventListener(ButtonEvent.DRAG_OUT, onCallbackEvent);
                object.addEventListener(ButtonEvent.RELEASE, onCallbackEvent);
                object.addEventListener(ButtonEvent.RELEASE_OUTSIDE, onCallbackEvent);
                object.addEventListener(ButtonEvent.STATE_UP, onCallbackEvent);
                object.addEventListener(ButtonEvent.STATE_OVER, onCallbackEvent);
                object.addEventListener(ButtonEvent.STATE_DOWN, onCallbackEvent);
            }
        ...
        private static function dispatchButtonEvent(button:InteractiveObject, type:String, event:MouseEvent = null):void
        {
            ...
            // however, there are always listeners when including callbacks
            if( !button.hasEventListener(type) ) {
                return;
            }
            
            var buttonEvent:MouseEvent;
            if(event == null) {
                buttonEvent = new MouseEvent(type, false, false, button.mouseX, button.mouseY);
            } else {
                buttonEvent = new MouseEvent(type, false, false, button.mouseX, button.mouseY, event.relatedObject,
        ...
        private static function onMouseDown(event:MouseEvent):void
        {
            ...
            dispatchButtonEvent(button, ButtonEvent.PRESS, event);
            dispatchButtonEvent(button, ButtonEvent.STATE_DOWN, event);
        }
        ...
        private static function onMouseMove(event:MouseEvent):void
        {
            for(var i:* in buttonIndex) {
                ...
                dispatchButtonEvent(button, ButtonEvent.DRAG, event);
            }
        }
        ...
        private static function onRollOver(event:MouseEvent):void
        {
            ...
            if(buttonIndex[button] != null) {
                ...
                dispatchButtonEvent(button, ButtonEvent.DRAG_OVER, event);
                dispatchButtonEvent(button, ButtonEvent.STATE_DOWN, event);
                
            } else if(!event.buttonDown) {
                
                dispatchButtonEvent(button, ButtonEvent.STATE_OVER, event);
            }
        ...
        private static function onRollOut(event:MouseEvent):void
        {
            ...
            if(buttonIndex[button] != null) {
                ...
                dispatchButtonEvent(button, ButtonEvent.DRAG_OUT, event);
                dispatchButtonEvent(button, ButtonEvent.STATE_OVER, event);
                
            } else if(!event.buttonDown) {
                
                dispatchButtonEvent(button, ButtonEvent.STATE_UP, event);
            }
        ...
        private static function onMouseUp(event:MouseEvent):void
        {
            ...
            if(buttonIndex[button] == null) {
                ...
                dispatchButtonEvent(button, ButtonEvent.ROLL_OVER, event);
                dispatchButtonEvent(button, ButtonEvent.STATE_OVER, event);
            }
        }
        ...
        private static function onRelease(event:Event):void
        {
            ...
            for(var i:* in buttonIndex) {
                ...
                if(buttonIndex[i]) {
                    ...
                    dispatchButtonEvent(button, ButtonEvent.RELEASE);
                    dispatchButtonEvent(button, ButtonEvent.STATE_OVER);
                    
                } else {
                    
                    dispatchButtonEvent(button, ButtonEvent.RELEASE_OUTSIDE);
                    dispatchButtonEvent(button, ButtonEvent.STATE_UP);
                }
